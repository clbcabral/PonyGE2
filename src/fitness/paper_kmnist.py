...
class paper_kmnist(base_ff):
    ...
    def load_data(self):
        ...
        ds = tfds.load('kmnist', batch_size=-1)
        dsnp = tfds.as_numpy(ds)

        train_images, test_images, train_labels, test_labels = dsnp['train']['image'], dsnp['test']['image'], dsnp['train']['label'], dsnp['test']['label']
